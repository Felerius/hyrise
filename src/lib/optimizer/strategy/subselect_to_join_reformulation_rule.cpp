...
//  }
  ...
  // join predicate.
  bool found_primary_join_predicate = false;
  for (auto it = join_predicates.begin(), end = join_predicates.end(); it != end; ++it) {
    const auto& predicate = std::static_pointer_cast<BinaryPredicateExpression>(*it);
    if (predicate->predicate_condition == PredicateCondition::Equals) {
      std::iter_swap(join_predicates.begin(), it);
      found_primary_join_predicate = true;
      break;
    }
  }

  if (!found_primary_join_predicate) {
    return _apply_to_inputs(node);
  }
